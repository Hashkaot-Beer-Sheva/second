...
const normalizeBuildingNumber = (v) => { const value = clean(v); const match = value.match(/^(\d+)$/); return match ? String(Number(match[1])) : value; };
const normalizeEntrance = (v) => { const value = clean(v); const map = { a:'א', b:'ב', c:'ג', d:'ד', e:'ה', f:'ו', g:'ז', h:'ח', i:'ט', j:'י' }; return map[value.toLowerCase()] || value; };
const altApartmentNumber = (v) => { const value = clean(v); const slash = value.match(/\s+\d+\s*\/\s*(\d+)\s*$/); if (slash) return slash[1]; const backslash = value.match(/\s+(\d+)\s*\\\s*\d+\s*$/); return backslash ? backslash[1] : ''; };
const altAddressFixes = new Set(['גוש עציון|89|39','גוש עציון|93|54','השלום|131|1','השלום|133|1','השלום|133|19','השלום|133|20','השלום|135|2','השלום|135|14','וינגייט|2|7','חנה סנש|14|6','רוטנברג|12|13']);
const unifiedAddressOverrides = { '090':'חנה סנש 02/06', '0A2':'גוש עציון 77/06', '087':'גוש עציון 91/06' };
...
const ownerGroup = (owner) => /השקעות דרום|וייס זאב דוד/.test(owner||'') ? 'Weiss' : /קרן אבירם|מוריס דוד דניאל|מוריס דויד דניאל|קרן אבירם בעמ|רוקון נאמנים/.test(owner||'') ? 'Morris' : 'Other';
...
export function buildData(raw) {
  ...
  easy.forEach(row=>{ const id=clean(row.UniquID||row['UniquID ']); const a=byUnifiedEasyId.get(id); if(!a)return; a.easyRent=Object.fromEntries(Object.entries(row).map(([k,v])=>[k,clean(v)])); a.buildingNumber=normalizeBuildingNumber(row['Street Number']);a.altAddress=clean(row.AltAddress);const fixKey=`${clean(row.Street)}|${a.buildingNumber}|${String(row.Appt||'').replace(/^0+(?=\d)/,'')}`;a.apartmentNumber=altAddressFixes.has(fixKey)?(altApartmentNumber(a.altAddress)||clean(row['Sub A']||row.Appt)):clean(row['Sub A']||row.Appt);a.entrance=normalizeEntrance(row.Entrance);a.address=[row.Street,a.buildingNumber].filter(Boolean).map(clean).join(' ');a.fullAddress=[a.address,a.apartmentNumber&&`Apt ${a.apartmentNumber}`,a.entrance&&`Entrance ${a.entrance}`].filter(Boolean).join(', '); byEasy.set(id,a); });
  for (const a of apartments) { const override = unifiedAddressOverrides[a.id]; if (!override) continue; const parsed = override.match(/^(.*?)\s+(\d+)\s*\/\s*(\d+)$/); if (!parsed) continue; a.address = `${clean(parsed[1])} ${normalizeBuildingNumber(parsed[2])}`; a.buildingNumber = normalizeBuildingNumber(parsed[2]); a.apartmentNumber = normalizeBuildingNumber(parsed[3]); a.easyRent.Street = clean(parsed[1]); a.easyRent['Street Number'] = a.buildingNumber; a.fullAddress = [a.address, `Apt ${a.apartmentNumber}`, a.entrance && `Entrance ${a.entrance}`].filter(Boolean).join(', '); }
  ...
  for (const a of apartments) {
    ...
    const number = normalizeBuildingNumber(a.easyRent['Street Number'] || a.buildingNumber);
  ...
  for (const a of apartments) {
    ...
    a.entrance = normalizeEntrance(building.entrance);
  ...
  const coords=raw.coordinates||[],buildingRows=apartments.map(a=>{const renterValues=Object.values(a.renter),owner=clean(renterValues[1]||a.renter.Owner||a.easyRent['Owner 1']);const cls=clean(a.easyRent.Class),streetNumber=normalizeBuildingNumber(a.buildingNumber),entrance=normalizeEntrance(a.entrance);const street=clean(a.easyRent.Street||a.address.split(/\s+\d+/)[0]);
    ...
    const exactEntrance=coords.find(c=>normalizeBuildingNumber(c.streetNumber)===streetNumber&&key(c.street)===key(street)&&entrance&&key(normalizeEntrance(c.entrance))===key(entrance));
    const exactBuilding=coords.find(c=>normalizeBuildingNumber(c.streetNumber)===streetNumber&&key(c.street)===key(street));
